...
def load_models():
    ...
    btrp = graphlab.load_model(DATA_PATH +
                               'boosted_trees_regression_for_phones')
    # boosted_trees_regression_for_apparel
    btra = graphlab.load_model(DATA_PATH +
                               'boosted_trees_regression_for_apparel')
    ...
    # neural network trained on the 1.2 million images of the ImageNet
    # Challenge.
    deep_learning_model = graphlab.load_model(DATA_PATH + 'imagenet_model')

    # [boosted_trees_category_classifier, topic_model_phones,
    # topic_model_apparel, topic_model_home, vectorizer_phones,
    # vectorizer_apparel, vectorizer_home, boosted_trees_regression_for_phones,
    # boosted_trees_regression_for_apparel, boosted_trees_regression_for_home,
    # similar_images_for_phones, similar_images_for_apparel,
    # similar_images_for_home, deep_learning_model]
    return [btcc, tmp, tma, tmh, vp, va, vh, btrp, btra, btrh, sip, sia, sih,
            deep_learning_model]


def load_data():
    # Load prices for nearest neighbors
    phones = graphlab.load_sframe(DATA_PATH + 'phones_with_ids_s')
    home = graphlab.load_sframe(DATA_PATH + 'home_with_ids_s')
    apparel = graphlab.load_sframe(DATA_PATH + 'apparel_with_ids_s')
